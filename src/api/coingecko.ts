...
export async function fetchTopCoins(limit: number = 20): Promise<Coin[]> {
  try {
    const url = `${COINGECKO_BASE_URL}/coins/markets?vs_currency=usd&per_page=${limit}&page=1&sparkline=false&price_change_percentage=1h%2C24h%2C7d%2C30d%2C90d%2C180d%2C270d%2C1y`;
    ...
    throw new CoinGeckoAPIError(`Unexpected error fetching top coins: ${(error as Error).message}`);
  }
}
...
export async function fetchCoinChart(coinId: string, days: number = 7): Promise<CoinChart> {
  try {
    ...
    throw new CoinGeckoAPIError(`Unexpected error fetching coin chart: ${(error as Error).message}`);
  }
}
...
export async function fetchCoinSearch(query: string): Promise<Coin[]> {
  try {
    ...
    throw new CoinGeckoAPIError(`Unexpected error searching coins: ${(error as Error).message}`);
  }
}
...
export async function fetchExchangeRates(base: string = 'USD'): Promise<ExchangeRates> {
  try {
    ...
    throw new ExchangeAPIError(`Unexpected error fetching exchange rates: ${(error as Error).message}`);
  }
}
...
export async function convertCurrency(from: string, to: string): Promise<CurrencyPair> {
  try {
    ...
    throw new ExchangeAPIError(`Unexpected error converting currency: ${(error as Error).message}`);
  }
}
...
export async function fetchMarketSummary(topCoins: Coin[]): Promise<{ totalMarketCap: number; total24hVolume: number; btcDominance: number; activeCoins: number }> {
  ...
  try {
    ...
    const totalCapExcludingBTCETH = totalMarketCap - btcCap - ethCap;
    
    if (totalCapExcludingBTCETH > 0) {
  ...
  const activeCoins = topCoins.length;
